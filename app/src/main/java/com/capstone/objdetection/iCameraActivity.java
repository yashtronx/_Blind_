...
public abstract class iCameraActivity extends Activity
        ...
        this.textToSpeech = new TextToSpeech(this, new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                if (status == TextToSpeech.SUCCESS) {
                    I_LOGGER.i("onCreate", "TextToSpeech is initialised");
                } else {
                    I_LOGGER.e("onCreate", "Cannot initialise text to speech!");
                }
            }
        });
        ...
        try {
            ...
            imageConverter =
                    new Runnable() {
                        @Override
                        public void run() {
                            iImageUtils.convertYUV420ToARGB8888(
                                    yuvBytes[0],
                                    yuvBytes[1],
                                    yuvBytes[2],
                                    previewWidth,
    ...
    public synchronized void onStart() {
        I_LOGGER.d("onStart " + this);
        super.onStart();
    }

    @Override
    public synchronized void onResume() {
        I_LOGGER.d("onResume " + this);
        super.onResume();
    ...
    public synchronized void onPause() {
        I_LOGGER.d("onPause " + this);

        if (!isFinishing()) {
            I_LOGGER.d("Requesting finish");
            finish();
        }
    ...
    public synchronized void onStop() {
        I_LOGGER.d("onStop " + this);
        super.onStop();
    }

    @Override
    public synchronized void onDestroy() {
        I_LOGGER.d("onDestroy " + this);
        super.onDestroy();
    }
    ...
    private void requestPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (shouldShowRequestPermissionRationale(PERMISSION_CAMERA) ||
                ...
                Toast.makeText(iCameraActivity.this,
    ...
    private String chooseCamera() {
        ...
        try {
            for (final String cameraId : manager.getCameraIdList()) {
                ...
                I_LOGGER.i("Camera API lv2?: %s", useCamera2API);
                return cameraId;
            }
        } catch (CameraAccessException e) {
            I_LOGGER.e(e, "Not allowed to access camera");
        }
    ...
    protected void setFragment() {
        ...
        iCameraConnectionFragment camera2Fragment =
                iCameraConnectionFragment.newInstance(
                        new iCameraConnectionFragment.ConnectionCallback() {
                            @Override
                            public void onPreviewSizeChosen(final Size size, final int rotation) {
                                previewHeight = size.getHeight();
                                previewWidth = size.getWidth();
                                iCameraActivity.this.onPreviewSizeChosen(size, rotation);
                            }
                        },
                        this,
                        getLayoutId(),
    ...
    protected void fillBytes(final Plane[] planes, final byte[][] yuvBytes) {
        ...
        for (int i = 0; i < planes.length; ++i) {
            ...
            if (yuvBytes[i] == null) {
                I_LOGGER.d("Initializing buffer %d at size %d", i, buffer.capacity());
    ...
    private List<iClassifier.Recognition> currentRecognitions;

    protected void toSpeech(List<iClassifier.Recognition> recognitions) {
        ...
        if (currentRecognitions != null) {
            ...
            final Set<iClassifier.Recognition> intersection = new HashSet<>(recognitions);
    ...
    private void speak() {
        ...
        for (int i = 0; i < currentRecognitions.size(); i++) {
            iClassifier.Recognition recognition = currentRecognitions.get(i);
